...
try_download http://ignum.dl.sourceforge.net/project/libpng/01-libpng-master/1.4.3/libpng-1.4.3.tar.gz
...
./configure --prefix=$CONFIGURE_PREFIX $IMAGEMAGICK_ARGUMENTS --with-gs-font-dir=$CONFIGURE_GS_FONT/fonts --without-fontconfig
make
sudo make install
cd ..
